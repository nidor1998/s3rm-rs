...
async fn e2e_versioned_bucket_creates_delete_markers() {
    ...
    let config = TestHelper::build_config(vec![&format!("s3://{bucket}/versioned/"), "--force"]);
...
async fn e2e_delete_all_versions() {
    ...
    //          (creates 2 versions each); delete some via normal API (creates
    //          delete markers).
    ...
    assert!(!result.has_error, "Pipeline should complete without errors");
    // Each object has 2 versions, so 20 total deletions expected
    assert_eq!(
        result.stats.stats_deleted_objects, 20,
        "Should delete all 20 versions (10 objects x 2 versions)"
    );
